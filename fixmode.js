...
rep("No conversation with Sasha yet on this deal — ask her something below.",
...
rep(A, B);
rep("body: JSON.stringify({ dealCode:c.dealCode, name:c.name, email:c.email, phone:c.phone, message:text, autoSend:c.sashaAutoSend, history:c.sashaChat, researchNotes:c.researchNotes })",
    "body: JSON.stringify({ mode: talking ? 'talk' : 'send', dealCode:c.dealCode, name:c.name, email:c.email, phone:c.phone, message:text, autoSend: talking ? false : c.sashaAutoSend, history: thread, researchNotes:c.researchNotes })");
...
if (hi === -1) { console.log('MISS hint'); process.exit(1); }
